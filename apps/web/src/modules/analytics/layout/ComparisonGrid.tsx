...
export const ComparisonGrid = forwardRef<HTMLDivElement, ComparisonGridProps>(function ComparisonGrid(
  ...
  return (
    <div
      ref={ref}
      className={cn("grid min-h-[70vh] items-stretch gap-px bg-white/10 lg:grid-cols-2", className)}
    >
...
function ComparisonPanel({ label, accent, children }: { label: string; accent: string; children: ReactNode }) {
  return (
    <section className="min-w-0 overflow-hidden bg-[#10131f]">
      <div className="sticky top-0 z-10 border-b border-white/10 bg-[#10131f]/95 px-4 py-3 backdrop-blur">
        <div className={cn("mb-2 h-1 rounded-full", accent)} />
        <p className="truncate text-xs font-black uppercase tracking-[0.18em] text-slate-500">Comparison context</p>
        <h3 className="truncate text-base font-black text-slate-100">{label}</h3>
      </div>
